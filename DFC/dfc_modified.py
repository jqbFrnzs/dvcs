		...
		if command != '':
			...
			if i < 2:
				if comm.lower() == 'get':
					command = 'get'
					continue
				elif comm.lower() == 'list_stage_local':
					command = 'list_local'
					continue
				elif comm.lower() == 'list_vcs_local':
					command = 'list_vcs_local'
				elif comm.lower() == 'create':
					command = 'create'
					continue
				elif comm.lower() == 'init':
					command = 'init'
					continue
				elif comm.lower() == 'list_remote':
					command = 'list_remote'
					continue
				elif comm.lower() == 'put':
					command = 'put'
	...
	while True:
		...
		# LIST_STAGE_LOCAL
			...
			if vcs.is_vcs_initialized():
				...
				print(vcs.get_working_dir_file_list())

		# LIST_VCS_LOCAL
		elif command.lower() == 'list_vcs_local':
			if vcs.is_vcs_initialized():
				print('Files stored locally in version control directory:')
